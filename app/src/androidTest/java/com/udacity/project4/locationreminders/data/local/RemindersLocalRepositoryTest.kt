...
class RemindersLocalRepositoryTest {
    ...
    @Test
    fun insertEqualsRetrieve() = runTest {
        val reminder = ReminderDTO("Title", "Description", "Location", 19.0, 20.2)

        remindersLocalRepository.saveReminder(reminder)
        val reminder2: Result.Success<ReminderDTO> =
            remindersLocalRepository.getReminder(reminder.id) as Result.Success

        assertThat(reminder2.data, `is`(reminder))
    }

    @Test
    fun noReminderError() = runTest {
        val reminder = ReminderDTO("Title", "Description", "Location", 19.0, 20.2)
        val id = reminder.id
        remindersLocalRepository.saveReminder(reminder)
